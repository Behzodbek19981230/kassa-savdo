...
export function OrderPaymentFields({ orderData, onOrderUpdate, totalAmount: _totalAmount = 0, refreshTrigger = 0 }: OrderPaymentFieldsProps) {
    ...
    const usdRate = orderData?.exchange_rate != null ? Number(orderData.exchange_rate) : USD_RATE;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const errorMessage =
                error?.response?.data?.detail || error?.message || 'Ma\'lumotlarni saqlashda xatolik';
            ...
            <div className='flex-1 overflow-y-auto p-3 sm:p-4 bg-white'>
                ...
                <div className='grid grid-cols-3 gap-2 sm:gap-3 mb-4'>
                    ...
                    >
                        ...
                        <p
                            className={`text-lg sm:text-xl font-bold ${remaining > 0 ? 'text-rose-600' : 'text-emerald-600'}`}
                        >
                            {Math.max(0, remaining).toLocaleString()}{' '}
                            <span
                                className={`text-xs font-normal ${remaining > 0 ? 'text-rose-500' : 'text-emerald-500'}`}
                            >
                                UZS
                            </span>
                        </p>
