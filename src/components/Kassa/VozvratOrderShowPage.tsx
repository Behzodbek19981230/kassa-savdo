...
export function VozvratOrderShowPage() {
    ...
    const usdRate = vozvrat_order?.exchange_rate != null ? Number(vozvrat_order.exchange_rate) : USD_RATE;
            ...
            <div className='bg-white rounded-lg shadow-md p-3 sm:p-4 mb-4'>
                <div className='flex items-center justify-between mb-3 pb-2 border-b border-gray-200'>
                    <h2 className='text-lg sm:text-xl font-bold text-gray-800'>
                        Tovar qaytarish #{vozvrat_order.id}
                    </h2>
                    <div className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                        vozvrat_order.is_karzinka 
                ...
                <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3'>
                    ...
                    <div className='bg-gradient-to-br from-blue-50 to-indigo-50 p-2 rounded border border-blue-200'>
                        ...
                        <p className='text-xs text-gray-600'>
                            {vozvrat_order.client_detail?.phone_number || ''}
                        </p>
                        {vozvrat_order.client_detail?.total_debt && Number(vozvrat_order.client_detail.total_debt) > 0 && (
                            <p className='text-[10px] text-red-600 font-semibold mt-0.5'>
                                Qarz: {Number(vozvrat_order.client_detail.total_debt).toLocaleString()}
                            </p>
                        )}
                    </div>
                        ...
                        {vozvrat_order.date ? (
                            <>
                                <p className='font-bold text-gray-800 text-sm'>
                                    {new Date(vozvrat_order.date).toLocaleDateString('ru-RU', {
                                        year: 'numeric',
                                        month: '2-digit',
                                        day: '2-digit',
                                    }).replace(/\//g, '.')}
                    ...
                    <div className='bg-gradient-to-br from-amber-50 to-orange-50 p-2 rounded border border-amber-200'>
                        ...
                        <p className='font-bold text-gray-800 text-sm'>
                            {vozvrat_order.filial_detail?.name || 'Noma\'lum'}
                        </p>
                    </div>
                    ...
                    <div className='bg-gradient-to-br from-indigo-50 to-blue-50 p-2 rounded border-2 border-indigo-300'>
                        ...
                        <p className='font-bold text-indigo-700 text-base mb-0.5'>
                            {(Number(vozvrat_order.summa_total_dollar || 0) * usdRate).toLocaleString()} UZS
                        </p>
                    ...
                    {Number(vozvrat_order.discount_amount || 0) > 0 && (
                        <div className='bg-gradient-to-br from-rose-50 to-red-50 p-2 rounded border border-rose-200'>
                            ...
                            <p className='font-bold text-rose-700 text-base'>
                                {Number(vozvrat_order.discount_amount || 0).toLocaleString()} UZS
                            </p>
                        </div>
                    )}
                            ...
                            <div className='grid grid-cols-2 sm:grid-cols-4 gap-2'>
                                {Number(vozvrat_order.summa_naqt || 0) > 0 && (
                                    <div className='bg-white p-2 rounded border border-violet-100'>
                                        ...
                                        <p className='font-bold text-gray-800 text-xs'>
                                            {Number(vozvrat_order.summa_naqt).toLocaleString()} UZS
                                        </p>
                                    </div>
                                )}
                                {Number(vozvrat_order.summa_dollar || 0) > 0 && (
                                ...
                                {Number(vozvrat_order.summa_transfer || 0) > 0 && (
                                    <div className='bg-white p-2 rounded border border-violet-100'>
                                        ...
                                        <p className='font-bold text-gray-800 text-xs'>
                                            {Number(vozvrat_order.summa_transfer).toLocaleString()} UZS
                                        </p>
                                    </div>
                                )}
                                {Number(vozvrat_order.summa_terminal || 0) > 0 && (
                                    <div className='bg-white p-2 rounded border border-violet-100'>
                                        ...
                                        <p className='font-bold text-gray-800 text-xs'>
                                            {Number(vozvrat_order.summa_terminal).toLocaleString()} UZS
                                        </p>
                                    </div>
                                )}
                            </div>
                                    ...
                                    return (
                                        <>
                                            ...
                                            <tr key={`header-${group.model_id}`} className='bg-blue-100 border-b border-blue-200'>
                                                <td colSpan={9} className='px-3 py-2 sm:px-4 sm:py-3 text-sm font-bold text-blue-800'>
                                                    {group.model}
                                                </td>
                                            </tr>
