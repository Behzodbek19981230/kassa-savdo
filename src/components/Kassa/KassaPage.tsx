...
export function KassaPage({ orderId, readOnly = false, updateMode = false, isVozvratOrder = false }: KassaPageProps) {
    ...
    const handleBranchChange = (branchId: number | null) => {
        ...
        setSelectedBranchCategory(null); // Reset branch category when branch changes
    };
    const handleBranchCategoryChange = (branchCategoryId: number | null) => setSelectedBranchCategory(branchCategoryId);
            ...
            prev.map((item) => {
                if (item.id === id) {
                    ...
                    return {
                        ...item,
                        quantity: newQty,
                        totalPrice: newQty * (item.price_sum || parseFloat(item.real_price || '0')),
                    };
                }
                return item;
            }),
    ...
    const totalAmount =
        (orderId ?? orderData?.id) ? totalAmountFromCart : cart.reduce((sum, item) => sum + (item.totalPrice || 0), 0);
                        ...
                        <VozvratPaymentModal
                            ...
                            totalAmount={
                                (orderId ?? orderData?.id)
                                    ...
                                    : cart.reduce((sum, item) => sum + (item.totalPrice || 0), 0) / exchangeRate
                            }
                            totalCount={
                                (orderId ?? orderData?.id)
