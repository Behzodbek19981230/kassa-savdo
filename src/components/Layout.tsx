...
export function Layout({ children, onBack, showBackButton = true }: LayoutProps) {
    ...
    useEffect(() => {
        // Start global notes websocket so it's active even when panel is closed
        startNotesWs();
        return () => {
            stopNotesWs();
        };
    ...
    return (
        <div className='flex flex-col h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50  '>
            ...
            <header className='bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-500 text-white px-2 sm:px-3 py-2 shrink-0 shadow-lg'>
                <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2'>
                            ...
                            <ul className='flex items-center gap-2'>
                                <li>
                                    <NavLink
                                        to={ROUTES.HOME}
                                        className={({ isActive }) =>
                                            `px-2 py-1 rounded-md text-xs font-semibold transition flex items-center gap-1.5 whitespace-nowrap shrink-0 ${isActive ? 'bg-white/20 text-white' : 'text-white/90 hover:bg-white/10'
                                            }`
                                        }
                                    >
                                ...
                                <li>
                                    <NavLink
                                        ...
                                        className={({ isActive }) =>
                                            `px-2 py-1 rounded-md text-xs font-semibold transition flex items-center gap-1.5 whitespace-nowrap ${isActive ? 'bg-white/20 text-white' : 'text-white/90 hover:bg-white/10'
                                            }`
                                        }
                    ...
                    <div className='flex items-center flex-wrap gap-1.5 sm:gap-2 w-full sm:w-auto justify-end'>
                        ...
                        <div className='flex items-center gap-2'>
                            {user?.order_filial && (
                                <div className='hidden sm:flex items-center gap-1.5 bg-white/20 px-2 py-1 rounded-lg backdrop-blur-sm ml-2'>
                                    ...
                                    <span className='text-[10px] font-semibold'>{user.order_filial_detail?.name}</span>
                                </div>
                            )}
                            <div
                                className={`hidden md:flex items-center gap-1.5 bg-green-600 px-2 py-1 rounded-lg shadow-md ${isSuperadmin && shouldShowExchangeRateDialog
                        ...
                        {/* Kassir Profili */}
                        {(kassir || user) && (
                            <div className='hidden sm:flex items-center gap-1.5 bg-white/20 px-2 py-1 rounded-lg backdrop-blur-sm'>
                                ...
                                <div className='text-[10px]'>
                                    ...
                                    <div className='text-[9px] opacity-80 mt-0.5 whitespace-nowrap'>
                        ...
                        <div className='relative' ref={ddRef}>
                            <div
                                ...
                                className='flex items-center gap-1.5 bg-white/20 px-2 py-1 rounded-lg backdrop-blur-sm shadow-sm cursor-pointer relative'
                            >
                            ...
                            {dropdownOpen && (
                                <div className='absolute right-0 mt-1.5 w-[320px] p-1 z-50 rounded-lg border border-white/20 bg-slate-900/85 backdrop-blur-sm text-white shadow-lg'>
                                    <p className='px-2 py-1 text-[11px] font-semibold text-white/80'>Bildirishnomalar</p>
                                        ...
                                        {combinedUnreadNotes.length === 0 ? (
                                            ...
                                            <div className='space-y-1 pb-1'>
                                                {combinedUnreadNotes.map((note) => (
                                                    <div
                                                        key={note.id}
                                                        className='cursor-pointer rounded-lg border border-white/10 bg-slate-800/70 p-0 hover:bg-slate-800/80 transition-colors'
                                                        onClick={(e) => {
                                                            e.preventDefault();
                                                            e.stopPropagation();
                                                    ...
                                                    >
                                                        <div className='w-full p-1.5'>
                                                            <div className='w-full flex items-start justify-between gap-1.5'>
                                                                <p className='text-xs font-semibold leading-4 line-clamp-1 text-white'>
                                                                    {note.title || 'Sarlavha'}
                                                                </p>
                                                                <span
                                                                    className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${note.status === 'expired'
                                    ...
                                    {combinedUnreadNotes.length > 0 && (
                                        <div className='pt-1.5 border-t border-white/10 mt-1'>
                                            <Button
                                                variant='ghost'
                                                className='w-full h-7 text-[10px] text-white/90 hover:text-white hover:bg-white/10'
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    e.stopPropagation();
                                                    markAllRead.mutate();
            ...
            <button
                ...
                className='fixed right-0 top-1/2 -translate-y-1/2 z-40 flex items-center gap-1 rounded-l-lg border border-r-0 border-amber-300 bg-amber-500 px-1.5 py-2 text-white shadow-md transition-all hover:px-2 hover:shadow-lg'
                title='Eslatmalar'
            >
