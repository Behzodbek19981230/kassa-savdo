    ...
    const overallTotals = useMemo(() => {
        ...
        for (const g of groups) {
            ...
            for (const o of items) {
                ...
                totalTolangan += parseFloat(o.summa_total_dollar || '0') || 0;
                                ...
                                {groups.length > 0 && groups[0]?.items?.[0] && (
                                    <tr className='bg-blue-50'>
                                        ...
                                        <td className='p-1 text-right font-semibold text-blue-700 text-xs'>
                                            {(
                                                formatMoney(overallTotals.totalZakaz.toFixed(2))
                                            )}
                                        </td>
                                        <td className='p-1 text-right font-semibold text-xs'>
                                            {
                                                formatMoney(overallTotals.totalTolangan.toFixed(2))
                                            }
                                        </td>
                                        <td className='p-1 text-right font-semibold text-xs'>
                                        ...
                                        <td className='p-1 text-right font-semibold text-red-600 text-xs'>
                                            {formatMoney(overallTotals.totalBugungiQarz.toFixed(2))}
                                        </td>
                                        <td className='p-1 text-right font-semibold text-red-600 text-xs'>
                                            {formatMoney((overallTotals.totalUmumiyQarz).toFixed(2))}
                                        </td>
                                        {(roles.isAdmin || roles.isSuperAdmin) && (
                                            <td className='p-1 text-right font-semibold text-green-600 text-xs'>
                                                {formatMoney(overallTotals.totalFoyda.toFixed(2))}
                                            </td>
                                        )}
                                        ...
                                        {(roles.isAdmin || roles.isSuperAdmin) && (
                                            <td className='p-1 text-right font-semibold text-xs'>
                                                {formatMoney(overallTotals.totalKeshbek.toFixed(2))}
                                            </td>
                                        )}
                                        <td className='p-1 text-right font-semibold text-xs'></td>
                                    </tr>
