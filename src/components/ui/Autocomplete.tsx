...
export function Autocomplete({
    ...
    const filteredOptions = React.useMemo(() => {
        ...
        return options.filter(
            (opt) => typeof opt.label === 'string' ? opt.label.toLowerCase().includes(query) : opt.label()?.toString().toLowerCase().includes(query) || opt.value.toLowerCase().includes(query),
        );
    ...
    return (
        <Popover.Root open={disabled ? false : open} onOpenChange={disabled ? undefined : setOpen}>
            ...
            <Popover.Portal>
                <Popover.Content
                    className={cn(
                        'z-50 w-[var(--radix-popover-trigger-width)] rounded-lg border-2 border-indigo-200 bg-white shadow-lg',
                ...
                >
                    {/* Search Input */}
                    <div className='p-1.5 border-b border-indigo-100 shrink-0'>
                    ...
                    >
                        ...
                        {searchQuery.trim() &&
                            !filteredOptions.some(
                                (opt) =>
                                    typeof opt.label === 'string' ? opt.label.toLowerCase() === searchQuery.toLowerCase() : opt.label()?.toString().toLowerCase() === searchQuery.toLowerCase() ||
