import { Eye, Loader2, ArrowRight, Search, FilterX, CheckCircle2 } from 'lucide-react';
		...
		<div className='p-2 sm:p-3 min-h-full'>
			<div className='bg-white rounded-xl shadow-xl p-2 sm:p-3 min-h-[400px] border border-gray-100 overflow-hidden'>
				<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2'>
					<h2 className='text-xl font-bold text-gray-800'>Mijozdan qarzdorlik</h2>
				</div>
								...
								{groups.length > 0 && groups[0]?.items?.[0] && (
									<tr className='bg-blue-50'>
										...
										<td className='p-1 text-right font-semibold text-blue-700 text-xs'>
											{(
												overallTotals.totalZakaz / (groups[0]?.items?.[0]?.exchange_rate || 1)
											).toFixed(2)}
										</td>
										<td className='p-1 text-right font-semibold text-xs'>
											{(
												overallTotals.totalTolangan /
										...
										<td className='p-1 text-right font-semibold text-red-600 text-xs'>
											{(
												overallTotals.totalBugungiQarz /
												(groups[0]?.items?.[0]?.exchange_rate || 1)
											).toFixed(2)}
										</td>
										<td className='p-1 text-right font-semibold text-red-600 text-xs'>
											{(
												overallTotals.totalUmumiyQarz /
												(groups[0]?.items?.[0]?.exchange_rate || 1)
											).toFixed(2)}
										</td>
										{(roles.isAdmin || roles.isSuperAdmin) && (
