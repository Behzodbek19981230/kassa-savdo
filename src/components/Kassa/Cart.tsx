...
export function Cart({
	...
	const loadOrderProducts = useCallback(async () => {
		...
		try {
			...
			const mapped = filtered.map((p: any) => ({
				...
				id: String(p.id),
				quantity: p.count ?? 0,
				price_dollar: parseFloat(p.price_dollar || '0'),
				price_sum: parseFloat(p.price_sum || '0'),
				totalPriceDollar: parseFloat(p.price_dollar || '0') * (p.count ?? 0),
				totalPrice: parseFloat(p.price_sum || '0') * (p.count ?? 0),
			}));
			setCartItemsFromApi(mapped as CartItem[]);
		} catch (error) {
	...
	const totalAmount = displayItems.reduce((sum, item) => sum + (item.price_sum ?? 0) * (item.count ?? 0), 0);
	const totalAmountDollar = displayItems.reduce((sum, item) => sum + (item.price_dollar ?? 0) * (item.count ?? 0), 0);
							...
							<div className='flex items-center gap-1.5 shrink-0'>
								<div className='text-right'>
									...
									<div className='text-[10px] text-gray-500'>
										{formatMoney((item.price_sum ?? 0) * (item.count ?? 0))} UZS
									</div>
								</div>
								{!readOnly && (
									<>
