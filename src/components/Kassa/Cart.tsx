...
export function Cart({
	...
	// Order-history-product dan CartItem ga transform (API: id, order_history, branch_detail, model_detail, type_detail, size_detail, count, given_count, real_price, unit_price)
	const transformOrderProductToCartItem = useCallback((op: any): CartItem => {
		const productName = [op.branch_detail?.name, op.model_detail?.name, op.type_detail?.name, op.size_detail?.size]
			.filter(Boolean)
			.join(' ')
			.trim();
		const quantity = op.given_count != null ? op.given_count : op.count ?? 0;
		const price = parseFloat(op.real_price || op.unit_price || op.wholesale_price || '0');
		return {
			id: String(op.id),
			productId: op.product ?? op.id,
			name: productName || `Savdo mahsulot #${op.id}`,
			price,
			stock: op.count ?? 0,
			unit: op.size_detail?.unit_detail?.name || 'dona',
			quantity,
			totalPrice: quantity * price,
			image: op.product?.images?.[0]?.file ?? op.images?.[0]?.file,
			branchName: op.branch_detail?.name ?? undefined,
			modelName: op.model_detail?.name ?? undefined,
			typeName: op.type_detail?.name ?? undefined,
			size: op.size_detail?.size ?? undefined,
			unitCode: op.size_detail?.unit_detail?.code ?? undefined,
			branchId: op.branch ?? undefined,
			modelId: op.model ?? undefined,
			typeId: op.type ?? undefined,
			sizeId: op.size ?? undefined,
			isFavorite: false,
		};
	}, []);
